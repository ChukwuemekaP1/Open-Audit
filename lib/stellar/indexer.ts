...
import {
  createFileIngestionStateStore,
  createMemoryIngestionStateStore,
  type IngestionStateSnapshot,
  type IngestionStateStore,
} from "./ingestion-state";
import {
  startCaptiveCoreIndexer,
  type CaptiveCoreControls,
  type CaptiveCoreSupervisorOptions,
} from "./captive-core";
...
export interface StreamingIndexerOptions {
   ...
   */
  maxQueueSize?: number;
  /** Optional durable state store for cursor/ledger checkpoints. */
  stateStore?: IngestionStateStore;
  /** Optional lookback hint when no stored cursor exists. */
